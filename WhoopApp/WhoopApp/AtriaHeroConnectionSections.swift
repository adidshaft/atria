    ...
    private var actionButtons: some View {
        HStack(spacing: 10) {
            if statusStore.state.status != .connected {
                Button(action: onShowSetupGuide) {
    ...
    private var connectionButton: some View {
        ...
        case .poweredOff, .disconnected:
            Button {
                ...
                Label(statusStore.state.status == .poweredOff ? "Bluetooth" : "Scan",
                      systemImage: statusStore.state.status == .poweredOff ? "bolt.slash.circle" : "bolt.horizontal.circle")
                    .font(.subheadline.weight(.semibold))
                    .lineLimit(1)
                    .minimumScaleFactor(0.82)
