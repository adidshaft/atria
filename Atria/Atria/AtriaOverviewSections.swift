...
struct AtriaWeeklyReportSheet: View {
    ...
    /// recovery days plot -- a nil day is a gap in the dots AND a break in
    /// the line (contiguousDayRuns, 2026-08-06 audit fix: the line used to
    /// bridge missed days, inventing readings), never invented.
    /// Points are shaped OUTSIDE any render block (perf gate: no compactMap
    /// in some-View bodies).
    private var weekRecoveryPoints: [AtriaDetailChartPoint] {
        (report.recoverySeries ?? []).compactMap { day in
            guard let recovery = day.recovery else { return nil }
            return AtriaDetailChartPoint(day: day.day,
                                         value: Double(recovery),
                                         tint: Metrics.electricGreen)
        }
    }

    private func weekRecoveryChart(_ points: [AtriaDetailChartPoint]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Chart {
                ForEach(points.contiguousDayRuns(), id: \.point.day) { entry in
                    LineMark(x: .value("Day", entry.point.day, unit: .day),
                             y: .value("Recovery", entry.point.value),
                             series: .value("Recovery run", "r\(entry.runID)"))
                        .interpolationMethod(.linear)
                        .foregroundStyle(Metrics.electricGreen)
                    PointMark(x: .value("Day", entry.point.day, unit: .day),
                              y: .value("Recovery", entry.point.value))
                        .foregroundStyle(Metrics.recoveryColor(Int(entry.point.value.rounded())))
                }
            }
...
// Internal (not private) since the 2026-08-06 audit fix: the live Today
// screen mounts this row -- previously its only presenter was the unmounted
// legacy Overview tree, leaving the monthly report unreachable.
struct AtriaMonthlyReportHighlightRow: View, Equatable {
...
struct AtriaMetricDetailSheet: View {
    ...
    // to an honest "still building" value so the pre-existing call sites keep
    // compiling unchanged. (2026-08-06: audit fix — the dead orphaned
    // AtriaVitalsTabContent/AtriaOverviewReadinessSection call sites were
    // deleted; AtriaTodayScreen and AtriaHealthScreen remain.)
...
private struct AtriaDetailPeriodSummary: Equatable, Sendable {
    ...
    private static func metricUnit(for unit: String) -> AtriaMetricUnit {
        ...
        // 2026-08-06 audit fix: "y" is the fitness-age delta series (signed
        // years vs calendar age). Routing it to .years keeps the sign and
        // unit intact and off the strain formatter's 0-21 clamp.
        case "y": return .years
        // The remaining fallthrough is unit "" — strain, deliberately
        // unitless, so .strain is the correct formatter for it.
