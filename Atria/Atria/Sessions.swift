...
final class SessionStore: ObservableObject {
    ...
    func logBaselineMaturityFromLaunchIfRequested(arguments: [String] = ProcessInfo.processInfo.arguments) {
        ...
        AtriaDebugLog("ATRIADBG baseline_maturity sessions=%d resting_samples=%d resting_mean=%@ resting_sd=%@ hrv_baseline_samples=%d hrv_required=%d hrv_ready=%d latest_validated_hrv=%d recovery_personal_ready=%d recovery_validated_ready=%d",
              baseline.sessions,
              baseline.restingSampleCount,
    ...
    func logGateReadinessFromLaunchIfRequested(arguments: [String] = ProcessInfo.processInfo.arguments) {
        ...
        let rows: [(gate: String, status: String, blocker: String)] = [
            ...
            ("C", latestReferenceValidatedHRV != nil && baseline.hrvSampleCount >= PersonalBaseline.trustedMinimumSamples ? "ready" : "learning", "validated_hrv_baseline_\(baseline.hrvSampleCount)_of_\(PersonalBaseline.trustedMinimumSamples)"),
    ...
    func logGateStatusFromLaunchIfRequested(arguments: [String] = ProcessInfo.processInfo.arguments) {
        ...
        logGateStatus("C", status: recoveryHighReady ? "ready" : "learning",
                      evidence: "validated_hrv_baseline=\(baseline.hrvSampleCount)/\(PersonalBaseline.trustedMinimumSamples); latest_validated_hrv=\(latestReferenceValidatedHRV ?? 0)")
    ...
    private func logBoundedLargeStoreGateStatus(mode: String,
        ...
        let externalBlocked = [
            ...
            hrvValidated == 0 ? "C:validated_hrv_baseline_\(baseline.hrvSampleCount)_of_\(PersonalBaseline.trustedMinimumSamples)" : nil,
        ...
        logGateStatus("C", status: recoveryHighReady ? "ready" : "learning",
                      evidence: "validated_hrv_baseline=\(baseline.hrvSampleCount)/\(PersonalBaseline.trustedMinimumSamples); hrv_baseline_ready=\(hrvBaselineReady ? 1 : 0); latest_validated_hrv=\(latestReferenceValidatedHRV ?? 0)")
        ...
        logGateETrainingDiagnostics(rest: rest,
                                    ...
                                    confirmedWorkouts: confirmedWorkoutRecords,
                                    confirmedSleeps: confirmedSleepRecords)
        let boundedTrend90 = trendSummaryFast(rest: rest, maxHR: profile.maxHR, days: 90)
        let boundedTrend90Blockers = trendBlockers(summary: boundedTrend90, hrvValidated: hrvValidated)
        let boundedTrend90AnomalyFlags = trendAnomalyFlags(boundedTrend90.anomalies)
        let boundedTrend90AnomalySource = boundedTrend90.anomalySource
        let boundedTrend90AnomalyDays = boundedTrend90.anomalySampleDays
        let boundedLocalWindows = (boundedTrend90.avgRHR != nil || boundedTrend90.avgStrain != nil) ? 1 : 0
        ...
        AtriaDebugLog("ATRIADBG trend_fast_local windows=%d local_windows=%d rhr_points=%d strain_points=%d recovery_points=%d hrv_points=%d trend90_confidence=%@ trend90_coverage_days=%d trend90_required_coverage_days=%d trend90_coverage_percent=%d trend90_anomaly_source=%@ trend90_anomaly_days=%d hrv_reference_gated=%d status=%@ blockers=%@",
              1,
              boundedLocalWindows,
              boundedTrend90.avgRHR == nil ? 0 : 1,
              boundedTrend90.avgStrain == nil ? 0 : 1,
              boundedTrend90.avgRecovery == nil ? 0 : 1,
              boundedTrend90.avgHRV == nil ? 0 : 1,
              boundedTrend90.confidence,
              boundedTrend90.coverageDays,
              boundedTrend90.requiredCoverageDays,
              boundedTrend90.coveragePercent,
        ...
        logGateStatus("F", status: boundedGateFStatus,
                      evidence: "bounded_large_store=1; trend_replay=fast_local_summary; local_non_hrv_trends_ready=\(boundedLocalTrendReady ? 1 : 0); local_non_hrv_windows=\(boundedLocalWindows); trend90_coverage_days=\(boundedTrend90.coverageDays); trend90_required_coverage_days=\(boundedTrend90.requiredCoverageDays); trend90_required_coverage_percent=70; trend90_coverage_percent=\(boundedTrend90.coveragePercent); trend90_sessions=\(boundedTrend90.sessions); trend90_recovery_points=\(boundedTrend90.avgRecovery == nil ? 0 : 1); trend90_hrv_points=\(boundedTrend90.avgHRV == nil ? 0 : 1); trend90_rhr_points=\(boundedTrend90.avgRHR == nil ? 0 : 1); trend90_strain_points=\(boundedTrend90.avgStrain == nil ? 0 : 1); trend90_anomalies=\(boundedTrend90.anomalies.count); trend90_anomaly_flags=\(boundedTrend90AnomalyFlags); trend90_anomaly_source=\(boundedTrend90AnomalySource); trend90_anomaly_days=\(boundedTrend90AnomalyDays); hrv_reference_gated=\(hrvValidated == 0 ? 1 : 0); trend_blockers=\(boundedTrend90Blockers)")
        logGateStatus("G", status: gateGStatus,
    ...
    private func logExecutionPriority(hrvValidated: Int,
        ...
        if hrvValidated == 0 {
            ...
            externalBlocked.append("C:validated_hrv_baseline_\(hrvBaselineSamples)_of_\(PersonalBaseline.trustedMinimumSamples)")
            if currentRRCaptureBlocked {
                localBlocked.append(currentRRBlocker)
            }
        } else if hrvBaselineSamples < PersonalBaseline.trustedMinimumSamples {
            localBlocked.append("C:personal_baseline_hrv_\(hrvBaselineSamples)_of_\(PersonalBaseline.trustedMinimumSamples)")
        }
