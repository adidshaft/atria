...
struct AtriaLiveActivityAttributes: ActivityAttributes {
    public struct ContentState: Codable, Hashable {
        ...
        // `timerAnchor` is an adjusted start date for the *moving* timer; it
        // shifts forward by every completed pause. While paused, the frozen
        // `elapsedDuration` is rendered instead of a running wall-clock timer.
