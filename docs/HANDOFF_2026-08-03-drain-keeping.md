...
4. **Remaining drain-keeping hardening** — ✅ timer-driven re-arm (P3), ✅ connected-slice teardown fix (P4), ✅ charge-resume (P5), ✅ flush-debt tracker (P6) all DONE (committed, sim-green, see §7b). **Left:** verify BGProcessing windows actually fire on-device; retire the non-convergent full drain. (Design doc + task #22.)
...
**Remaining task #22:** verify BGProcessing windows actually fire on-device (P0b/P5); confirm P3.1 `lastMaintenanceReArmAt` populates under a background/HR-sparse soak; retire the non-convergent full-drain path entirely (design doc #6).
