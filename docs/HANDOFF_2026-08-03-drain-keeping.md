...
3. **Build task #23 — onboarding wipe-vs-sync choice** (the user's new feature request). First-run, when a connected strap already has banked data, show a one-line two-option choice: **"Start fresh · clears the strap, ready in seconds (Recommended)"** vs **"Bring my history · syncs ~Xh in the background"** (ETA computed from `pending_records`). **Productionize the trim out of `#if DEBUG`** into an onboarding-gated path where the user's tap on "Start fresh" IS the sacrificial-loss confirmation (no launch flags); keep all safety. See memory `atria-onboarding-strap-wipe-choice` + `docs/drain-keeping-flush-design.md`.
